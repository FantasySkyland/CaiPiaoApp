...
class StrictLineReader implements Closeable {
  ...
  /**
   * Buffered data is stored in {@code buf}. As long as no exception occurs, 0 <= pos <= end and the
   * data in the range [pos, end) is buffered for reading. At end of input, if there is an
   * unterminated line, we set end == -1, otherwise end == pos. If the underlying
   ...
   *        supported.
   * @throws NullPointerException if {@code in} or {@code charset} is null.
   * @throws IllegalArgumentException if {@code capacity} is negative or zero or the specified
  ...
  /**
   * Closes the reader by closing the underlying {@code InputStream} and marking this reader as
   * closed.
   * 
  ...
  /**
   * Reads new input data into the buffer. Call only with pos == end or end == -1, depending on the
   * desired outcome if the function throws.
   */
